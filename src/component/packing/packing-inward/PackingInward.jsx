...
import { deletePackingInward, getAllPackingInward } from "@/lib/fetcher";

const PackingInward = () => {
  const { user, loading } = useAuth();

  const [packingInward, setPackingInward] = useState([]);
  ...
  /* ================= FETCH PACKING INWARD ================= */
  const fetchPackingInward = async (page = 1, size = 10, searchText = "") => {
    try {
      setInwardLoading(true);
      setServerError(false);

      const res = await getAllPackingInward(page, size, searchText);

      if (res?.success) {
        const data = res.data || [];
        setPackingInward(data);
      ...
      console.error("PACKING INWARD FETCH ERROR:", error);
      setServerError(true);
  ...
  const handleDelete = async (id) => {
    try {
      await deletePackingInward(id);
      toast.success("Packing inward deleted successfully");

      const isLastItemOnPage = packingInward.length === 1 && pageNo > 1;
      const targetPage = isLastItemOnPage ? pageNo - 1 : pageNo;

      if (isLastItemOnPage) {
        setPageNo(targetPage);
      }
      fetchPackingInward(targetPage, pageSize, search);
    } catch (err) {
  ...
  const goToPage = (p) => {
    ...
    setPageNo(next);
    fetchPackingInward(next, pageSize, search);
  };
  ...
  const handlePageSizeChange = (e) => {
    ...
    setPageSize(size);
    setPageNo(1);
    fetchPackingInward(1, size, search);
  };
  ...
  return (
    <div className="flex-1 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 pt-16 lg:pt-4 overflow-hidden">
      ...
      <div className="mb-4 shrink-0">
        <h1 className="text-2xl font-semibold text-black">Packing Inward</h1>
      </div>
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 shrink-0">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const timeout = setTimeout(() => {
                setPageNo(1);
                if (!user) return;
                fetchPackingInward(1, pageSize, value);
              }, 500);
        ...
        <Link
          href="/packing/packing-inward/add-inward"
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  ...
                  <tr>
                    <td colSpan="8" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            fetchPackingInward(pageNo, pageSize, search)
                          }
                    ...
                    <td colSpan="8" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <p className="text-gray-400 text-sm">
                          There are no packing inwards to display.
                        </p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  packingInward.map((inward, index) => (
                    <tr
                      key={inward.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm">
                        {inward.product?.product_name || "-"}
                      </td>
                      <td className="px-6 py-4 text-sm">
                        {inward?.fitter?.user_name || "-"}
                      </td>
                      <td className="px-6 py-4 text-sm">
                        {inward?.receive_gurus || "-"}
                      </td>
                      <td className="px-6 py-4 text-sm">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-4">
                          <FiEye
                            ...
                            onClick={() =>
                              router.push(
                                `/packing/packing-inward/view-packing-inward/${inward.id}`
                              )
                            }
                          />
                          <FiEdit
                            ...
                            onClick={() =>
                              router.push(
                                `/packing/packing-inward/edit-packing-inward/${inward.id}`
                              )
                            }
                          />
                          <IoTrashOutline
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t bg-white">
          <div className="text-sm text-gray-600">
            Showing <b>{packingInward.length}</b> of <b>{totalItems}</b>
          </div>
