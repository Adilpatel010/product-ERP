...
// import CreateSupplierModel from "../models/packingUser/CreateSupplierModel";
// import DeleteConfirmModal from "../models/DeleteConfirmModal";
// import EditSupplierModel from "../models/packingUser/EditSupplierModel";
// import ViewSupplierModel from "../models/packingUser/ViewSupplierModel";
...
import EditPackingUser from "@/component/models/packing-user/EditPackingUser";

const PackingUser = () => {
  const { user, loading } = useAuth();
  const [packingUser, setPackingUser] = useState([]);
  const [search, setSearch] = useState("");
  const [packingUserLoading, setPackingUserLoading] = useState(true);
  ...
  const [editPackingUser, setEditPackingUser] = useState(null);
  ...
  /* ================= FETCH SUPPLIERS ================= */
  const fetchPackingUsers = async (page = 1, size = 10, searchTerm = "") => {
    try {
      setPackingUserLoading(true);
      setServerError(false);
      const res = await getAllPackingUser(page, size, searchTerm);

      if (res && res.data) {
        setPackingUser(res.data);
      ...
      console.error("FETCH PACKING USER ERROR:", err);
      setServerError(true);
      toast.error("Failed to connect to server");
    } finally {
      setPackingUserLoading(false);
    }
  };
  ...
  const handleEdit = async (id) => {
    try {
      const res = await getPackingUserById(id);
      setEditPackingUser(res.data);
      setOpenEditModel(true);
    } catch (err) {
      toast.error("Failed to load packingUser data");
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await deletePackingUser(id);
      toast.success("Packing user deleted");

      const isLastItem = packingUser.length === 1 && pageNo > 1;
      const targetPage = isLastItem ? pageNo - 1 : pageNo;

      if (isLastItem) setPageNo(targetPage);
      fetchPackingUsers(targetPage, pageSize, search);
    } catch (err) {
  ...
  const goToPage = (p) => {
    ...
    setPageNo(next);
    fetchPackingUsers(next, pageSize, search);
  };
  ...
  const handlePageSizeChange = (e) => {
    ...
    setPageSize(size);
    setPageNo(1);
    fetchPackingUsers(1, size, search);
  };
  ...
  return (
    <div className="flex-1 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 pt-16 lg:pt-4 overflow-hidden">
      ...
      <div className="mb-4 shrink-0">
        <h1 className="text-2xl font-semibold text-black">Packing Fitter</h1>
      </div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const timeout = setTimeout(() => {
                setPageNo(1);
                if (!user) return;
                fetchPackingUsers(1, pageSize, value);
              }, 500);
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  ...
                  <tr>
                    <td colSpan="4" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <button onClick={() => fetchPackingUsers(pageNo, pageSize, search)} className="mt-2 text-secondary underline text-sm cursor-pointer">Retry</button>
                      </div>
                    </td>
                  </tr>
                ) : packingUser.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-gray-400 font-semibold text-lg">No Data Found</span>
                        <p className="text-gray-400 text-sm">There are no packing user to display.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  packingUser.map((packingUser, index) => (
                    <tr key={packingUser.id} className={`border-b whitespace-nowrap border-gray-200 hover:bg-gray-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                      <td className="px-6 py-4 text-sm font-semibold text-primary">{packingUser.user_name}</td>
                      <td className="px-6 py-4 text-sm">
                        {packingUser?.created_by?.username}
                        <span className="text-xs text-gray-400" style={{ marginLeft: "2px" }}>
                          ({packingUser?.created_by?.role})
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm ">
                        {packingUser?.updated_by?.username}
                        <span className="text-xs text-gray-400" style={{ marginLeft: "2px" }}>
                          ({packingUser?.updated_by?.role})
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex gap-4">
                          <FiEdit className="cursor-pointer text-primary hover:text-blue-600" onClick={() => handleEdit(packingUser.id)} />
                          <IoTrashOutline className="cursor-pointer text-red-500 hover:text-red-700" onClick={() => {
                            setSelectedId(packingUser.id); setOpenDeleteModel(true);
                          }} />
                        </div>
                      </td>
                    </tr>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t bg-white">
          <div className="text-sm text-gray-600">
            Showing <b>{packingUser.length}</b> of <b>{totalItems}</b>
          </div>
      ...
      <AddPackingUser
        ...
        fetchPackingUsers={fetchPackingUsers}
      />
      ...
      <EditPackingUser
        ...
        fetchPackingUsers={fetchPackingUsers}
        editPackingUserData={editPackingUser}
      />
      ...
      <DeleteConfirmModal
        ...
        title="Delete Packing User"
        message="Are you sure you want to delete this packing user? This action cannot be undone."
      />
