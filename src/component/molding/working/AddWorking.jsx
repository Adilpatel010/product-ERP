...
import { createWorking, searchMachine, searchProduct } from "@/lib/fetcher";
...
const AddWorking = () => {
    ...
    const isRowComplete = (row) => {
        return (
            row.product_id &&
            Number(row.qty_in_bag) > 0 &&
            Number(row.qty_in_kg) > 0
        );
    };
    ...
    const handleChange = (index, field, value) => {
        setRows((prev) => {
            ...
            if (value.split(".").length > 2) return prev;

            updated[index][field] = value;

            const bag = Number(updated[index].qty_in_bag) || 0;
            const kg = Number(updated[index].qty_in_kg) || 0;
    ...
    const handleSubmit = async (e) => {
        ...
        const items = rows
            .filter(
                (r) =>
                    r.product_id &&
                    Number(r.qty_in_bag) > 0 &&
                    Number(r.qty_in_kg) > 0
            )
            .map((r) => ({
                ...
                electricity_unit:
                    r.electricity_unit && Number(r.electricity_unit) > 0
                        ? Number(r.electricity_unit)
                        : null,
            }));
    ...
    return (
        <div className="flex-1 pt-16 lg:pt-4 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 overflow-hidden">
            ...
            <div className="flex flex-row justify-between items-center gap-3 mb-4">
                <h1 className="text-lg md:text-2xl font-semibold">Add Working</h1>
                <Link href="/molding"
        ...
        focus:ring-2 focus:ring-secondary/20`}
                ...
                gap-2 items-center border-b sm:border-0 pb-3 sm:pb-0 mb-3 sm:mb-0">
                            ...
                            <div className="relative w-full">
                                ...
                                <div className="absolute right-3 top-8.5 md:top-2.5 text-gray-500">
