...
import { deleteProduct, getAllProducts } from "@/lib/fetcher";
...
const Product = () => {
    ...
    /* ================= FETCH RAW INWARD ================= */
    const fetchproduct = async (page = 1, size = 10, searchText = "") => {
        try {
            setProductLoading(true);
            setServerError(false);
            const res = await getAllProducts(page, size, searchText);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            const isLastItemOnPage = product.length === 1 && pageNo > 1;
            const targetPage = isLastItemOnPage ? pageNo - 1 : pageNo;

            if (isLastItemOnPage) {
                setPageNo(targetPage);
            }
            fetchproduct(targetPage, pageSize, search);
        } catch (err) {
    ...
    const goToPage = (p) => {
        ...
        setPageNo(next);
        fetchproduct(next, pageSize, search);
    };
    ...
    return (
        <div className="flex-1 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 pt-16 lg:pt-4 overflow-hidden">
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col">
                        ...
                        <table className="min-w-200 w-full border-collapse">
                            <thead className="sticky whitespace-nowrap top-0 bg-secondary text-white z-10">
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-4 text-sm font-semibold">Color</th>
                                    <th className="text-left px-6 py-4 text-sm font-semibold">Lot in KG</th>
                                    <th className="text-left px-6 py-4 text-sm font-semibold">Total Gross in Lot</th>
                                    ...
                                    <tr>
                                        <td colSpan="7" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <button onClick={() => fetchproduct(pageNo, pageSize, search)} className="mt-2 text-secondary underline text-sm cursor-pointer">Retry</button>
                                            </div>
                                        </td>
                                    </tr>
                                ) : product.length === 0 ? (
                                    <tr>
                                        <td colSpan="7" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <p className="text-gray-400 text-sm">There are no products to display.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    product.map((p, index) => (
                                        <tr key={p.id} className={`border-b whitespace-nowrap border-gray-200 hover:bg-gray-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                                            <td className="px-6 py-4 text-sm font-semibold text-primary">{p.product_name || "-"}</td>
                                            <td className="px-6 py-4 text-sm">{p.color || "All Color"}</td>
                                            <td className="px-6 py-4 text-sm">{p.lot_in_kg}</td>
                                            <td className="px-6 py-4 text-sm">{p.total_gurus_lot}</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-4">
                                                    <FiEye className="cursor-pointer text-primary hover:text-blue-600" title="View"
                                                        onClick={() => router.push(`/product/view-product/${p.id}`)} />
                                                    <FiEdit
                                                        className="cursor-pointer text-primary hover:text-blue-600"
                                                        onClick={() =>
                                                            router.push(`/product/edit-product/${p.id}`)
                                                        }
                                                    />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t bg-white">
                    <div className="text-sm text-gray-600">
                        Showing <b>{product.length}</b> of <b>{totalItems}</b>
                    </div>
            ...
            <DeleteConfirmModal
                ...
                message="Are you sure you want to delete this record? This will remove all items associated with this product."
            />
        </div>
    );
};
