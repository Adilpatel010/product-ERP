...
import EditRawProductModel from "@/component/models/raw-product/EditRawProductModel";
import ViewRawProductModel from "@/component/models/raw-product/ViewRawProductModel";
...
const RawProduct = () => {
  ...
  // get all raw product
  const fetchRawProducts = async (page = 1, size = 10, search = "") => {
    try {
      setProductLoading(true);
      setServerError(false);
      const res = await getAllRawProducts(page, size, search);
      ...
      console.error("FETCH RAW PRODUCTS ERROR:", error);
      setServerError(true);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const isLastItemOnPage = products.length === 1 && pageNo > 1;
      ...
      fetchRawProducts(targetPage, pageSize, search);
    } catch (err) {
  ...
  const goToPage = (p) => {
    ...
    setPageNo(next);
    fetchRawProducts(next, pageSize, search);
  };
  ...
  const handlePageSizeChange = (e) => {
    ...
    setPageSize(size);
    setPageNo(1);
    fetchRawProducts(1, size, search);
  };
  ...
  return (
    <div className="flex-1 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 pt-16 lg:pt-4 overflow-hidden">
      ...
      <div className="mb-4 shrink-0">
        <h1 className="text-2xl font-semibold text-black">Raw Product</h1>
      </div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const timeout = setTimeout(() => {
                setPageNo(1);
                if (!user) return;
                fetchRawProducts(1, pageSize, value);
              }, 500);
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col">
                ...
                {serverError ? (
                  <tr>
                    <td colSpan="7" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            fetchRawProducts(pageNo, pageSize, search)
                          }
                    ...
                    <td colSpan="7" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <p className="text-gray-400 text-sm">
                          There are no raw products to display.
                        </p>
                      </div>
                    </td>
                  </tr>
                  ...
                  products.map((product, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm">{product.rate}</td>
                      <td className="px-6 py-4 text-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t bg-white">
          <div className="text-sm text-gray-600">
            Showing <b>{products.length}</b> of <b>{totalItems}</b>
          </div>
      ...
      <CreateRawProductModel
        ...
        fetchRawProducts={fetchRawProducts}
      />
      ...
      <EditRawProductModel
        ...
        fetchRawProducts={fetchRawProducts}
        editRawProductData={editProduct}
      />
      ...
      <DeleteConfirmModal
        ...
        message="Are you sure you want to delete this raw product? This action cannot be undone."
      />
    </div>
  );
};
