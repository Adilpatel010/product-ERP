...
import { deleteRawOutward, getAllRawOutward } from "@/lib/fetcher";
...
const RawOutward = () => {
    ...
    /* ================= FETCH RAW OUTWARD ================= */
    const fetchRawOutward = async (page = 1, size = 10, searchText = "") => {
        try {
            setOutwardLoading(true);
            setServerError(false);
            const res = await getAllRawOutward(page, size, searchText);

            if (res && res.data) {
                setRawOutward(res.data);
            ...
            console.error("RAW OUTWARD FETCH ERROR:", error);
            setServerError(true);
    ...
    const handleDelete = async (id) => {
        try {
            await deleteRawOutward(id);
            toast.success("Raw outward deleted successfully");

            const isLastItemOnPage = rawOutward.length === 1 && pageNo > 1;
            const targetPage = isLastItemOnPage ? pageNo - 1 : pageNo;

            if (isLastItemOnPage) {
                setPageNo(targetPage);
            }
            fetchRawOutward(targetPage, pageSize, search);
        } catch (err) {
    ...
    const goToPage = (p) => {
        ...
        setPageNo(next);
        fetchRawOutward(next, pageSize, search);
    };
    ...
    return (
        <div className="flex-1 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 pt-16 lg:pt-4 overflow-hidden">
            ...
            <div className="mb-4 shrink-0">
                <h1 className="text-2xl font-semibold text-black">Raw Outward</h1>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 shrink-0">
                <div className="relative w-full sm:w-72">
                    <input
                        type="text"
                        placeholder="Search by product or receiver"
                        value={search}
                        onChange={(e) => {
                            ...
                            const timeout = setTimeout(() => {
                                setPageNo(1);
                                if (!user) return;
                                fetchRawOutward(1, pageSize, value);
                            }, 500);
                ...
                <Link
                    href="/raw-material/raw-outward/add-outward"
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col">
                    ...
                    {outwardLoading ? (
                        ...
                        <table className="min-w-200 w-full border-collapse">
                            <thead className="sticky whitespace-nowrap top-0 bg-secondary text-white z-10">
                                <tr>
                                    <th className="text-left px-6 py-4 text-sm font-semibold">Outward Date</th>
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <button onClick={() => fetchRawOutward(pageNo, pageSize, search)} className="mt-2 text-secondary underline text-sm cursor-pointer">Retry</button>
                                            </div>
                                        </td>
                                    </tr>
                                ) : rawOutward.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <p className="text-gray-400 text-sm">There are no raw outwards to display.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    rawOutward.map((outward, index) => (
                                        <tr key={outward.id} className={`border-b whitespace-nowrap border-gray-200 hover:bg-gray-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                                            <td className="px-6 py-4 text-sm">{formatDate(outward.outward_date)}</td>
                                            <td className="px-6 py-4 text-sm">{outward.total_products || 0}</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-4">
                                                    <FiEye
                                                        ...
                                                        title="View"
                                                        onClick={() => router.push(`/raw-material/raw-outward/view-outward/${outward.id}`)}
                                                    />
                                                    <FiEdit
                                                        className="cursor-pointer text-primary hover:text-blue-600"
                                                        title="Edit"
                                                        onClick={() => router.push(`/raw-material/raw-outward/edit-outward/${outward.id}`)}
                                                    />
                                                    <IoTrashOutline
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t bg-white">
                    <div className="text-sm text-gray-600">
                        Showing <b>{rawOutward.length}</b> of <b>{totalItems}</b>
                    </div>
