...
export default function EditRawOutward() {
  ...
  const handleSubmit = async (e) => {
    ...
    const invalidRow = rows.find(
      (r) => r.product_id && (!r.qty || Number(r.qty) <= 0),
    );
    if (!products.length) {
    ...
    try {
      setUpdate(true);
      const res = await updateRawOutward(id, payload);
      if (res?.success) {
        toast.success("Raw outward updated successfully");
  ...
  return (
    <div className="flex-1 pt-16 lg:pt-4 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 overflow-hidden">
      ...
      <div className="flex flex-row justify-between items-center gap-3 mb-4">
        <h1 className="text-lg md:text-2xl font-semibold">Edit Raw Outward</h1>
        <Link
