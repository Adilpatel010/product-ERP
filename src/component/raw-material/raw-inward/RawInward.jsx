...
import { deleteRawInward, getAllRawInward } from "@/lib/fetcher";
...
const RawInward = () => {
    ...
    const [inwardLoading, setInwardLoading] = useState(true);
    ...
    /* ================= FETCH RAW INWARD ================= */
    const fetchRawInward = async (page = 1, size = 10, searchText = "") => {
        try {
            setInwardLoading(true);
            setServerError(false);
            const res = await getAllRawInward(page, size, searchText);

            if (res && res.data) {
                setRawInward(res.data);
            ...
            console.error("RAW INWARD FETCH ERROR:", error);
            setServerError(true);
    ...
    const handleDelete = async (id) => {
        try {
            await deleteRawInward(id);
            toast.success("Raw inward deleted successfully");

            const isLastItemOnPage = rawInward.length === 1 && pageNo > 1;
            const targetPage = isLastItemOnPage ? pageNo - 1 : pageNo;

            if (isLastItemOnPage) {
                setPageNo(targetPage);
            }
            fetchRawInward(targetPage, pageSize, search);
        } catch (err) {
    ...
    const goToPage = (p) => {
        ...
        setPageNo(next);
        fetchRawInward(next, pageSize, search);
    };
    ...
    return (
        <div className="flex-1 bg-grey h-dvh flex flex-col px-4 sm:px-6 py-4 pt-16 lg:pt-4 overflow-hidden">
            ...
            <div className="mb-4 shrink-0">
                <h1 className="text-2xl font-semibold text-black">Raw Inward</h1>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 shrink-0">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            const timeout = setTimeout(() => {
                                setPageNo(1);
                                if (!user) return;
                                fetchRawInward(1, pageSize, value);
                            }, 500);
                ...
                <Link
                    href="/raw-material/raw-inward/add-inward"
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col">
                        ...
                        <table className="min-w-200 w-full border-collapse">
                            <thead className="sticky whitespace-nowrap top-0 bg-secondary text-white z-10">
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-4 text-sm font-semibold">Inward Date</th>
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <button onClick={() => fetchRawInward(pageNo, pageSize, search)} className="mt-2 text-secondary underline text-sm cursor-pointer">Retry</button>
                                            </div>
                                        </td>
                                    </tr>
                                ) : rawInward.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <p className="text-gray-400 text-sm">There are no raw inwards to display.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    rawInward.map((inward, index) => (
                                        <tr key={inward.id} className={`border-b whitespace-nowrap border-gray-200 hover:bg-gray-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                                            <td className="px-6 py-4 text-sm font-semibold text-primary">{inward.supplier_name || "-"}</td>
                                            <td className="px-6 py-4 text-sm">{formatDate(inward.inward_date)}</td>
                                            <td className="px-6 py-4 text-sm">{inward.total_products || 0}</td>
                                            <td className="px-6 py-4 text-sm">
                                                {inward?.created_by?.username}
                                                <span className="text-xs text-gray-400" style={{ marginLeft: "2px" }}>
                                                    ({inward?.created_by?.role})
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-sm ">
                                                {inward?.updated_by?.username}
                                                <span className="text-xs text-gray-400" style={{ marginLeft: "2px" }}>
                                                    ({inward?.updated_by?.role})
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex gap-4">
                                                    <FiEye className="cursor-pointer text-primary hover:text-blue-600" title="View"
                                                        onClick={() => router.push(`/raw-material/raw-inward/view-inward/${inward.id}`)} />
                                                    <FiEdit
                                                        className="cursor-pointer text-primary hover:text-blue-600"
                                                        onClick={() =>
                                                            router.push(`/raw-material/raw-inward/edit-inward/${inward.id}`)
                                                        }
                                                    />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t bg-white">
                    <div className="text-sm text-gray-600">
                        Showing <b>{rawInward.length}</b> of <b>{totalItems}</b>
                    </div>
            ...
            <DeleteConfirmModal
                ...
                title="Delete Raw Inward"
                message="Are you sure you want to delete this record? This will remove all items associated with this inward."
            />
        </div>
    );
};

export default RawInward;