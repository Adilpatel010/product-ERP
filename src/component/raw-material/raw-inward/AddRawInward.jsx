...
import { createRawInward, searchRawProduct, searchSupplier } from "@/lib/fetcher";
...
const AddRawInward = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createRawInward(payload);

      if (res?.success) {
        toast.success("Raw inward created successfully");

        setTimeout(() => {
          router.push("/raw-material/raw-inward");
        }, 300);
      } else {
        toast.error(res?.message || "Failed to create raw inward");
      }
    } catch (error) {
      toast.error(
  ...
  const handleChange = (index, field, value) => {
    setRows((prev) => {
      ...
      if (isRowComplete(lastRow) && !updated.some((r, i) => i > lastIndex && isBlankRow(r))) {
            ...
            <div className="relative">
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Enter") {
                    e.preventDefault();
                    const supplier = suppliers[activeIndex];
                    if (supplier) {
                      setSearch(supplier.supplier_name);
        ...
        focus:ring-2 focus:ring-secondary/20`}
            ...
            {!selectedSupplier && (
              <>
                ...
                {!searchLoading && search && suppliers.length === 0 && (
                  <div

                    className="absolute z-50 w-full mt-1 px-3 py-2 text-sm text-gray-500 bg-white border border-gray-300 shadow-lg rounded-lg">
                    No supplier found
                  </div>
                )}
              </>
